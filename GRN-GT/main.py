...
train_dataset, val_dataset, test_dataset = load_dataset(path)
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_epoch == 0:
            ...
            if best_val_auroc < auroc:
                ...
                if args.feature_type == 'fusion':
                    test_auroc, _ = evaluate(model, test_dataloader, device, torch.cat((gene_embs, epr.to(device)), dim=1))
                elif args.feature_type == 'exp':
            ...
            if best_val_auprc < auprc:
                ...
                if args.feature_type == 'fusion':
                    _, test_auprc = evaluate(model, test_dataloader, device, torch.cat((gene_embs, epr.to(device)), dim=1))
                elif args.feature_type == 'exp':
